...
export default function BookForm({ onSubmit, initial = null, onCancel }) {
  ...
  const submit = (e) => {
    ...
    if (!title.trim() || !author.trim()) {
      setError("יש למלא כותרת ושם המחבר");
      return;
    }
    ...
    <div className="card">
      <form onSubmit={submit}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <strong>{initial ? "ערוך ספר" : "הוסף ספר חדש"}</strong>
          {onCancel && <button type="button" className="btn secondary" onClick={onCancel}>ביטול</button>}
        </div>
        <div className="form-row">
          <label className="small">כותרת *</label>
          <input className="input" value={title} onChange={e => setTitle(e.target.value)} />
        </div>
        <div className="form-row">
          <label className="small">מחבר *</label>
          <input className="input" value={author} onChange={e => setAuthor(e.target.value)} />
        </div>
        <div className="row">
          <div style={{flex:1}} className="form-row">
            <label className="small">שנה</label>
            <input className="input" value={year} onChange={e => setYear(e.target.value)} />
          </div>
          <div style={{width:140}} className="form-row">
            <label className="small">קטגוריה</label>
            <input className="input" value={category} onChange={e => setCategory(e.target.value)} />
          </div>
        </div>
        {error && <div style={{color:"red",marginBottom:8}}>{error}</div>}
        <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:8}}>
          <button type="submit" className="btn">{initial ? "שמור" : "הוסף"}</button>
        </div>
      </form>
    </div>
  );
